...
from mytools import MSELossIgnoreNaN_RNN
...
class SpatioTemporalLSTMCell(nn.Module):
    def __init__(self, in_channel, num_hidden, img_height, img_width, filter_size, stride, layer_norm):
        ...
        self._forget_bias = 1.0
        # 根据patch_size更新空间尺寸
        # 假设patch_size在调用时已经处理好（例如传入的img_height, img_width已经是经过patch转换后的尺寸）
        if layer_norm:
            self.conv_x = nn.Sequential(
...
class RNN(nn.Module):
    ...
    def __init__(self, num_layers, num_hidden, configs):
        ...
        self.input_frame_channel = configs.patch_size * configs.patch_size * configs.input_channel
        self.output_frame_channel = configs.patch_size * configs.patch_size * configs.output_channel
        ...
        self.MSE_criterion = MSELossIgnoreNaN_RNN()
        for i in range(num_layers):
            in_channel = self.input_frame_channel if i == 0 else num_hidden[i - 1]
            cell_list.append(
                SpatioTemporalLSTMCell(in_channel, num_hidden[i], h, w, configs.filter_size, configs.stride,
                                       configs.layer_norm)
            )
        self.cell_list = nn.ModuleList(cell_list)
        self.conv_last = nn.Conv2d(num_hidden[num_layers - 1], self.output_frame_channel, kernel_size=1, stride=1, padding=0,
                                   bias=False)
        # shared adapter
    ...
    def forward(self, frames, mask_true):
        ...
        for t in range(self.configs.total_length - 1):
            ...
            if t < time_schedule:
                ...
                net = mask_true[:, t - time_schedule] * true_frames + \
                          (1 - mask_true[:, t - time_schedule]) *  x_gen

            if self.configs.need_mask:
                mask_land = frames[:, t,
                            self.input_frame_channel - self.configs.patch_size ** 2: self.input_frame_channel]
                net = torch.concatenate((net, mask_land), dim=1)
        ...
        if self.training:
            ...
            loss = self.MSE_criterion(pred, target ) + self.configs.decouple_beta * decouple_loss
        else:
            ...
            loss = self.MSE_criterion(pred, target )
        return pred, loss
